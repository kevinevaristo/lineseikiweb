...
function addDynamicTable(table) {
    ...
    data.columns.forEach((col, i) => {
        headerRow.appendChild(dtBuildHeaderCell(col));
    });
    // Last header cell holds the per-column delete buttons row spacer
    const headerActions = document.createElement('th');
    headerActions.className = 'dt-col-actions';
    headerActions.innerHTML = '';
    headerRow.appendChild(headerActions);

    // Build column-action row (delete-column buttons under header)
...
function dtAddCol(btn) {
    ...
    // Add cell to each data row before the row-actions td
    dataRows.forEach(tr => {
...
function dtRemoveCol(btn) {
    ...
    // Index among action tds (excluding the trailing spacer)
...
async function saveProduct() {
    ...
    try {
        ...
        // 5b. Collect dynamic tables JSON
