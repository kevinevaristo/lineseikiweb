...
/**
 * Count new (unseen) subscribers
 */
public function count_unseen() {
    return $this->db->where('is_seen', 0)->count_all_results($this->table);
}

/**
 * Mark all subscribers as seen
 */
public function mark_all_seen() {
    $this->db->where('is_seen', 0);
    return $this->db->update($this->table, ['is_seen' => 1]);
}
}
