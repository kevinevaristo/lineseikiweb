...
class about_us_model extends CI_Model
{
    ...
    public function get_all_partners() {
        return $this->db->order_by('id', 'DESC')->get('tbl_partners')->result();
    }
    ...
    public function add_stat($data) {
        $insert_data = array(
            ...
            'stat_order' => isset($data['stat_order']) ? $data['stat_order'] : 0,
            'is_active' => 1,
            'created_at' => date('Y-m-d H:i:s')
        );
        
        $this->db->insert('tbl_stats', $insert_data);
        return $this->db->insert_id();
    }
    ...
    public function update_stat($id, $data) {
        ...
        if (isset($data['stat_order'])) {
            $update_data['stat_order'] = $data['stat_order'];
        }
        
        $this->db->where('id', $id);
        return $this->db->update('tbl_stats', $update_data);
    }
    
    /**
     * Update specific stat field
     */
    public function update_stat_field($id, $field, $value) {
        $data = array(
            $field => $value,
            'updated_at' => date('Y-m-d H:i:s')
        );
        ...
        $this->db->where('id', $id);
        return $this->db->update('tbl_stats', $data);
    }
    
    /**
     * Delete statistic (soft delete)
     */
    public function delete_stat($id) {
        $this->db->where('id', $id);
        return $this->db->delete('tbl_stats');
    }
    
    public function get_max_order() {
        $this->db->select_max('stat_order');
        $query = $this->db->get('tbl_stats');
        $result = $query->row();
        return $result->stat_order ?: 0;
    }
